...
def book_details(request, book_id):
    ...
    ##cabmio para que no de 404 si no existe el id del dlibro
...
def checkout_paypal(request, cart, orders):
    if request.user.is_authenticated():
        ...
        print('paypalrestsdk.configure')
        paypalrestsdk.configure({
            "mode": "sandbox",
            "client_id": "Ab1bYsKq-WZkC00UMu39wyL9pWVhXdhMZpYc5EUKm5VfBLUFce8ggZFP8jBx4ra939J7-9gfT4d8ELr1",
            "client_secret": "EPAJ2OOV8KQL99KPV7oE1jQlK-TMKIEXme9dBJXpUcW09upwcfdWQiyQcxE8EQvlxUUYqtAEfSg5Augr"})

        print('paypalrestsdk.Payment')
        print(str(total))
        print(items)
        ...
        if payment.create():
            ...
            cart_instance.save()
            print(' for link in payment.links:')
            for link in payment.links:
                if link.method == "REDIRECT":
                    redirect_url = str(link.href)
                    return redirect_url
        else:
            print(' return reverse(order_error)')
            print payment.error
...
def checkout_stripe(cart, orders, token):
    stripe.api_key = "sk_test_RfnVp2uAFBj7xJbNGVAHsewW"
    total = 0
    for order in orders:
        total += (order.book.price * order.quantity)
    status = True
